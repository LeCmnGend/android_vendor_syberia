...
function setup_vendor() {
    ...
    export SYBERIA_ROOT="$3"
    if [ ! -d "$SYBERIA_ROOT" ]; then
        echo "\$SYBERIA_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$SYBERIA_ROOT/$OUTDIR" ]; then
        mkdir -p "$SYBERIA_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$SYBERIA_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDBP="$SYBERIA_ROOT"/"$OUTDIR"/Android.bp
    export ANDROIDMK="$SYBERIA_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$SYBERIA_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$SYBERIA_ROOT"/vendor/syberia/build/tools/smali/baksmali.jar
        export SMALIJAR="$SYBERIA_ROOT"/vendor/syberia/build/tools/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$SYBERIA_ROOT"/vendor/syberia/build/tools/"$HOST"/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$SYBERIA_ROOT"/vendor/syberia/build/tools/"$HOST"/compact_dex_converter
    fi
    ...
    if [ ! -f "$SYBERIA_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$SYBERIA_TARGET" >/dev/null; then
...
function extract() {
    ...
    local OUTPUT_ROOT="$SYBERIA_ROOT"/"$OUTDIR"/proprietary
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            do
                ...
                if [ -a "$DUMPDIR"/"$PARTITION".new.dat ]; then
                    ...
                    python "$SYBERIA_ROOT"/vendor/syberia/build/tools/sdat2img.py "$DUMPDIR"/"$PARTITION".transfer.list "$DUMPDIR"/"$PARTITION".new.dat "$DUMPDIR"/"$PARTITION".img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$SYBERIA_ROOT"/"$OUTDIR"/radio
